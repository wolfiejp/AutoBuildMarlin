...
var config = {
  type:  'remote',
  host:  'http://www.thinkyhead.com/_marlin/config',
  path:  '_marlin/config'
};
/**/
...
window.configuratorApp = (function(){
  ...
  return {
          ...
          complete: function() {
            ...
            if (ajax_count >= config_file_list.length) {
              ...
              if (success_count < ajax_count)
                self.setMessage('Unable to load configurations.', 'error');
     ...
     *   .end    ending line
     *
     * #if    ... Push a simple condition to ifStack. Apply on #elif, #else, or #endif
     * #elif  ... All previous conditions are reversed, plus a new condition.
     * #else  ... All previous conditions are reversed, but no new condition.
     * #endif ... End of range for condition.
     *
     */
    initDependentGroups: function() {
      ...
      $.each(config_list, function(i, $v) {
        ...
        while((r = findBlock.exec(txt)) !== null) {
          ...
          var cond = r[2].replace(/[ \t]*\/\/.*$/, '');
          switch(r[1]) {
            case 'if':
              var code = self.conditionCode(cond);
            ...
            case 'else':
              var prev = '', line;
              $.each(ifStack, function(i,c) {
                if (prev != '') prev += ' || ';
                prev += c[0];
                line = c[1];
              });
              code = '!' + prev + (r[1] == 'elif' ? ' && ' + self.conditionCode(cond) : '');
              self.addDependentGroup(code, i, line, lineNum);
              break;
            ...
            case 'endif':
              ...
              if (c) {
                ...
                self.log("pop " + c[0], 5);
                self.addDependentGroup(cond, i, line, lineNum);
                if (r[1] == 'else') {
                  // Reverse all previous conditions and push to start
    ...
    addDependentGroup: function(cond, i, line, lineNum) {
    ...
    fileLoaded: function(filename, txt, wait) {
      ...
      switch(filename) {
        ...
        case config_file:
          if (has_boards) {
            ...
            if (!wait) cindex = 0;
            has_config = true;
            if (has_config_adv && wait) cindex = 2;
          }
          else {
            err = boards_file;
          }
      ...
      this.setMessage(err
        ? 'Please repair the "' + boards_file + '" file first!'
    ...
    getDefineInfo: function(name, cindex) {
      ...
      var debug_this = (name == 'EEPROM_SETTINGS');
      ...
      if (r !== null) {
        ...
        if (debug_this) self.log(name + " is a switch...");
      }
      else {
        ...
        if (r !== null) {
          ...
          if (debug_this) self.log(name + " is a list...");
        }
        else {
          ...
          if (r !== null) {
            ...
            if (debug_this) self.log(name + " is quoted...");
          }
          else {
            ...
            if (r !== null) {
              ...
              if (debug_this) self.log(name + " is plain...");
            }
          }
        }
      }
      ...
      if (info.type) {
        ...
        if (debug_this) self.log(name + " got a type...");
        ...
        if (debug_this) self.log(name + " end comment is " + eoltip);
        ...
        if (r = find.exec(txt)) {
          ...
          if (debug_this) self.log(name + " getting preceding comments");
        ...
        if (debug_this) this.log(name + ".enabled is " + info.enabled);
